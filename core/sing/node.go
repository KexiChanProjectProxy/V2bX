...
func getInboundOptions(tag string, info *panel.NodeInfo, c *conf.Options) (option.Inbound, error) {
	...
	// Parse sniff timeout
	var sniffTimeout badoption.Duration
	if c.SingOptions.SniffTimeout != "" {
		duration, err := time.ParseDuration(c.SingOptions.SniffTimeout)
		if err == nil {
			sniffTimeout = badoption.Duration(duration)
		}
	}
